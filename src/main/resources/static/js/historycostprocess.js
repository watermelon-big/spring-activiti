    $(document).ready(function(){
	    ...
	    var grid=$("#grid-data").bootgrid({
		    ...
		    ajax:true,
		    url:"getfinishcostprocess",
		    formatters: {
		    "process_instance_id": function(column, row){
		    	return row.costapply.process_instance_id;
		    },
		    "user_id":function(column, row){
		    	return row.costapply.user_id;
		    },
		    "cost_type":function(column, row){
		    	return row.costapply.cost_type;
		    },
			"cost_amount":function(column, row){
				return row.costapply.cost_amount;
			},
		    "apply_time":function(column, row){
		    	return row.costapply.apply_time;
		    },
		    "commands": function(column, row)
		    {
		            return "<button class=\"btn btn-xs btn-default ajax-link command-run1\" data-row-id=\"" + row.costapply.process_instance_id + "\">查看详情</button>";
		    }
	    	}
